...
import pyfits as fits  # http://www.astropy.org/
import numpy as np
...
while True:
    ...
    if centrecount <= galaxymin:
        break
    passes = 0
    for coords in maxcoordslist:
        if mask[coords]:
            curapradius = getapsize(coords)
            count = getcirclecount(coords, curapradius)
            if count > 0:
                catalog.append({'centre':coords,'peakcount':origdata[coords],'avcount':count,'mag':getmag(count),
...
newfitsfile("galactic_location.fits", maskgrid)
print 'Done.\n'
