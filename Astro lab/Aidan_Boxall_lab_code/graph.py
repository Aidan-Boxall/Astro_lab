...
for index in range(len(base[:-1])):
    ...
    elif index in range(9,47):  # plot middle straight line section
...
for index, value in enumerate(starterrlist):
    starterr.append((1.0/np.log(10))*(value/startcum[index]))
...
for index, value in enumerate(straighterrlist):
    straighterr.append((1.0/np.log(10))*(value/straightcum[index]))
...
for index, value in enumerate(enderrlist):
    enderr.append((1.0/np.log(10))*(value/endcum[index]))
...
plt.errorbar(endbase[:-1], endcum[:-1], enderr[:-1], c='black',fmt=' ')
